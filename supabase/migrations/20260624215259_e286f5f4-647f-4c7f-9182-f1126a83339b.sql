...
BEGIN
  -- Email: must be a valid-looking address; broker domains / generic inboxes
  -- are now treated as a SOFT signal (still counted, but won't pass ready check).
  ...
  -- Owner: present and non-empty. Broker-name no longer auto-disqualifies.
  ...
  ELSIF NOT has_owner AND NOT has_property
        AND NEW.owner_name ~* broker_owner_re THEN
    -- Only hard-block if it looks like a broker page with nothing else.
    NEW.readiness := 'low_confidence';
...
-- Rescue leads that were parked purely by the previous broker filter.
UPDATE public.leads
SET pipeline_stage = 'enriched',
    discovery_status = COALESCE(NULLIF(discovery_status, 'failed'), 'reachable'),
    discovery_attempt_count = 0,
    last_discovery_attempt_at = NULL,
    updated_at = now()
WHERE pipeline_stage IN ('needs_review')
  AND readiness IN ('low_confidence', 'needs_manual_review');